"""
Root Nutrient Heatmaps — Multi-Year Comparison

Generates side-by-side heatmaps showing root nutrient levels across phosphorus
...
Input:
- root_phosfert_merged.csv (requires Year, P_appln, and root nutrient columns)

Output:
- combined_root_heatmaps.png: multi-year root nutrient heatmaps with shared colorbar
- root_df_hm_050725.csv: cleaned/renamed data table
...
df = pd.read_csv('root_phosfert_merged.csv')
df.info()
df.head()
...
df.to_csv('root_df_hm_050725.csv', index=False)
...
plt.savefig('combined_root_heatmaps.png', bbox_inches='tight')
plt.show()
